...
const Quantity:React.FC<IQuantity> = ({ item }) => {
    ...
    const action = (type:string) => {
        if(type === 'add'){ 
            setTotal(total + 1)
            dispatch(updateTotalValue({type:'add',price:item.price, counter:total+1, id: item.id}))
        } else{ 
            ...
            setTotal(decrement) 
            dispatch(updateTotalValue({type:'remove',price:item.price, counter:total-1, id: item.id }))
        }
    }
